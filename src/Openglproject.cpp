	 ...
	 //   depthShader.use();
...

		{
			...
			if (ImGui::TreeNode((const char*)u8"显示设置"))
			{
				...
				ImGui::Checkbox((const char*)u8"启用光源跟随视角", &lightfollowcamera);
				if (!lightfollowcamera)
				{
					ImGui::SliderFloat((const char*)u8"光源角度X", &light_x, 0, 359.0);
					ImGui::SliderFloat((const char*)u8"光源角度Y", &light_y, 0, 359.0);
					ImGui::SliderFloat((const char*)u8"光源距离", &light_dep, 0, 50);
				}
				else if(lightfollowcamera)
				{
					ImGui::SliderFloat((const char*)u8"光源相对角度X", &light_rel_x, 0, 359.0);
					ImGui::SliderFloat((const char*)u8"光源相对角度Y", &light_rel_y, 0, 359.0);
				}
				
				//ImGui::SliderFloat((const char*)u8"光源相对摄像机角度Y", &light_y, -90.0, 90.0);
				//ImGui::Text((const char*)u8"角度x：", light_x);
				ImGui::SliderFloat((const char*)u8"环境光系数", &ambientStrength, 0.1, 1.0);
				ImGui::SliderFloat((const char*)u8"漫反射系数", &diffStrength, 0.1, 1.0);
			...
			if (ImGui::TreeNode((const char*)u8"附加网格"))
			{
				...
				if (render_Attach || attach_file !="")
				{
					...
					ImGui::Checkbox((const char*)u8"显示附加网格", &render_Attach);
					ImGui::SameLine();
			...
			{
				ImGui::SliderFloat((const char*)u8"设定计算点云边长", &points_side_length, ims.points_side_length_low, ims.points_side_length_up);
				ImGui::SliderInt((const char*)u8"设定计算点云密度", &size, ims.points_density_low, ims.points_density_up);
				ImGui::Checkbox((const char*)u8"保存计算结果(matchPoints.txt)", & write_txt);
				if (ImGui::Button((const char*)u8"重置计算"))                        // Buttons return true when clicked (most widgets return true when edited/activated)
				{
					POI_X = size;
					POI_Y = size;
					POI_Z = size;
			...
			{
				ImGui::SliderFloat((const char*)u8"球形剔除半径", &radius_cut, ims.radius_cut_low, ims.radius_cut_up);


				ImGui::SliderFloat((const char*)u8"上剔除面", &up_cut, ims.up_cut_low, ims.up_cut_up);
				ImGui::SliderFloat((const char*)u8"下剔除面", &down_cut, ims.down_cut_low, ims.down_cut_up);
				ImGui::SliderFloat((const char*)u8"右剔除面", &right_cut, ims.right_cut_low, ims.right_cut_up);
				ImGui::SliderFloat((const char*)u8"左剔除面", &left_cut, ims.left_cut_low, ims.left_cut_up);
				ImGui::SliderFloat((const char*)u8"后剔除面", &back_cut, ims.back_cut_low, ims.back_cut_up);
				ImGui::SliderFloat((const char*)u8"前剔除面", &front_cut, ims.front_cut_low, ims.front_cut_up);
